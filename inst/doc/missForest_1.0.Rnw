...
%\VignetteIndexEntry{missForest_1.0.Rnw}
...
\date{Friday, 13$^{\textrm{th}}$ of May, 2011}
\begin{document}
\maketitle
...
two values for the result of the imputation. The first value is the
normalized root mean squared error (NRMSE, see \cite{oba03}) for the
continuous part of the imputed data set, e.g., \texttt{Sepal.Length},
\texttt{Sepal.Width}, \texttt{Petal.Length} and \texttt{Petal.Width}. The
second value is the proportion of falsely classified entries (PFC) in the
categorical part of the imputed data set, e.g., \texttt{Species}. In both
cases good performance of \mF leads to a value around 1 and bad
performance to a value close to 0.
...
caveats at one specific data sets. In the above case of the \texttt{esoph}
data we can get the result of the fourth iteration by doing the following:
...
The returned result is now given by iteration 4. Quintessentially, there
